...
public class BeanMapTestCase extends AbstractTestMap {
     ...
     * replace: From: private \(.*\) some\(.*\); To: public \1 getSome\2Value() {
    ...
    /**
     * An object value that will be stored in the bean map as a value. Need to save
     * this externally so that we can make sure the object instances are equivalent
     * since getSampleValues() would otherwise construct a new and different Object
     * each time.
     **/
    ...
    /**
     * The mappings in a BeanMap are fixed on the properties the underlying bean
     * has. Adding and removing mappings is not possible, thus this method is
    ...
    /**
     * Need to override this method because the "clear()" method on the bean map
     * just returns the bean properties to their default states. It does not
     * actually remove the mappings as per the map contract. The default testClear()
     * methods checks that the clear method throws an UnsupportedOperationException
     * since this class is not add/remove modifiable. In our case though, we do not
    ...
    /**
     * Need to override this method because the "put()" method on the bean doesn't
     * work for this type of Map.
     */
    @Override
