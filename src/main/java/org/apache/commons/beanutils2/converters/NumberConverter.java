 ...
 * </ul>
 *
 * <h2>String Conversions (to and from)</h2> This class provides a number of ways in which number conversions to/from Strings can be achieved:
 * <ul>
 ...
 * </ul>
 * </li>
 * <li>If none of the above are configured the {@code toNumber(String)} method is used to convert from String to Number and the Number's {@code toString()}
 * method used to convert from Number to String.</li>
 * </ul>
 *
 * <p>
 * <strong>N.B.</strong>Patterns can only be specified using the <em>standard</em> pattern characters and NOT in <em>localized</em> form (see
 * {@link java.text.DecimalFormat}). For example to cater for number styles used in Germany such as {@code 0.000,00} the pattern is specified in the normal form
 * {@code 0,000.00</code> and the locale set to <code>Locale.GERMANY}
 *
...
public abstract class NumberConverter<N extends Number> extends AbstractConverter<N> {
    ...
    /**
     * Constructs a {@link Number} <em>Converter</em> that returns a default value if an error occurs.
     *
     * @param allowDecimals Indicates whether decimals are allowed
    ...
    /**
     * Gets the number format pattern used to convert Numbers to/from a {@link String} (or {@code null} if none specified).
     * <p>
    ...
    private Number toNumber(final Class<?> sourceType, final Class<?> targetType, final String value) {
        ...
        final String msg = toString(getClass()) + " cannot handle conversion from '" + toString(sourceType) + "' to '" + toString(targetType) + "'";
    ...
    /**
     * Convert any Number object to the specified type for this <em>Converter</em>.
     * <p>
