 ...
 */
public class OriginalWeakFastHashMap<K, V> extends HashMap<K, V> {
     ...
     */
    public OriginalWeakFastHashMap() {
        super();
        this.map = createMap();
    }
     ...
     */
    public OriginalWeakFastHashMap(final int capacity) {
        super();
     ...
     */
    public OriginalWeakFastHashMap(final int capacity, final float factor) {
        super();
     ...
     */
    public OriginalWeakFastHashMap(final Map<? extends K, ? extends V> map) {
        super();
    ...
    @Override
    public Object clone() {
        OriginalWeakFastHashMap<K, V> results = null;
        if (fast) {
            results = new OriginalWeakFastHashMap<>(map);
        } else {
            synchronized (map) {
                results = new OriginalWeakFastHashMap<>(map);
            }
    ...
    private abstract class CollectionView<E> implements Collection<E> {
        ...
        public void clear() {
            if (fast) {
                synchronized (OriginalWeakFastHashMap.this) {
                    map = createMap();
                }
            } else {
                synchronized (map) {
            ...
            public void remove() {
                ...
                if (fast) {
                    synchronized (OriginalWeakFastHashMap.this) {
                        ...
                        OriginalWeakFastHashMap.this.remove(lastReturned.getKey());
                        lastReturned = null;
                        expected = map;
                    }
                } else {
