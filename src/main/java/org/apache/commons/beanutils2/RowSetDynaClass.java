 ...
 * property of the corresponding name (optionally forced to lower case for
 * portability). There will be one {@link DynaBean} in the
 * {@code List</code> returned by <code>getRows()} for each row in the original
 * {@code ResultSet}.
 * </p>
 * <p>
 * In general, instances of {@link RowSetDynaClass} can be serialized and
 * deserialized, which will automatically include the list of {@link DynaBean}s
 * representing the data content. The only exception to this rule would be when
 * the underlying property values that were copied from the {@code ResultSet}
 * originally cannot themselves be serialized. Therefore, a
 * {@link RowSetDynaClass} makes a very convenient mechanism for transporting
 * data sets to remote Java-based application components.
 * </p>
 */
...
public class RowSetDynaClass extends JDBCDynaClass {
    ...
    /**
     * <p>
     * Limits the size of the returned list. The call to {@code getRows()} will
     * return at most limit number of rows. If less than or equal to 0, does not
     ...
     * {@code ResultSet} on which this {@link RowSetDynaClass} was based.
     * </p>
     */
    ...
    /**
     * <p>
     * Construct a new {@link RowSetDynaClass} for the specified {@code ResultSet}.
     * The property names corresponding to column names in the result set will be
     * lower cased.
     * </p>
     *
     * @param resultSet The result set to be wrapped
     * @throws NullPointerException if {@code resultSet} is {@code null}
     * @throws SQLException if the metadata for this result set cannot be
     *             introspected
     */
     ...
     * lower cased.
     * </p>
     * If {@code limit</code> is not less than 0, max <code>limit} number of rows
     ...
     * <p>
     * Construct a new {@link RowSetDynaClass} for the specified {@code ResultSet}.
     * The property names corresponding to the column names in the result set will
     * be lower cased or not, depending on the specified {@code lowerCase} value.
     * </p>
     * If {@code limit</code> is not less than 0, max <code>limit} number of rows
     * will be copied into the resultset.
     *
     * @param resultSet The result set to be wrapped
     ...
     * </p>
     * <p>
     * <strong>WARNING</strong> - If you specify {@code false} for
     * {@code lowerCase}, the returned property names will exactly match the column
     * names returned by your JDBC driver. Because different drivers might return
     * column names in different cases, the property names seen by your application
     * will vary depending on which JDBC driver you are using.
     * </p>
     *
     * @param resultSet The result set to be wrapped
     * @param lowerCase Should property names be lower cased?
    ...
    /**
     * <p>
     * Return a {@code List} containing the {@link DynaBean}s that represent the
     * contents of each {@code Row} from the {@code ResultSet} that was the basis of
     * this {@link RowSetDynaClass} instance. These {@link DynaBean}s are
     * disconnected from the database itself, so there is no problem with modifying
     * the contents of the list, or the values of the properties of these
     * {@link DynaBean}s. However, it is the application's responsibility to persist
    ...
    /**
     * <p>
     * Copy the column values for each row in the specified {@code ResultSet} into a
     * newly created {@link DynaBean}, and add this bean to the list of
    ...
    /**
     * <p>
     * Create and return a new {@link DynaBean} instance to be used for representing
     * a row in the underlying result set.
     * </p>
     *
