 ...
 * </ul>
 * <h3>String Conversions (to and from)</h3> This class provides a number of
 ...
 * using:
 * <ul>
 * <li>Either {@code setPattern(String)} or {@code setPatterns(String[])}</li>
 * </ul>
 * </li>
 * <li>Using the specified date pattern(s) for a specified Locale, configure
 * using:
 * <ul>
 * <li>{@code setPattern(String)} or {@code setPatterns(String[]) and...}</li>
 * <li>{@code setLocale(Locale)}</li>
 * </ul>
 * </li>
 * <li>If none of the above are configured the {@code toDate(String)} method is
 * used to convert from String to Date and the Dates's {@code toString()} method
 * used to convert from Date to String.</li>
 * </ul>
 * <p>
 * The <b>Time Zone</b> to use with the date format can be specified using the
 * {@link #setTimeZone(TimeZone)} method.
 *
 * @since 1.8.0
 */
public abstract class DateTimeConverter extends AbstractConverter {
     ...
     *
     * @param useLocaleFormat {@code true} if the format for the locale should be
    ...
    /**
     * Return the Time Zone to use when converting dates (or {@code null} if none
     * specified.
     *
    ...
    /**
     * Return the Locale for the <i>Converter</i> (or {@code null} if none
     * specified).
     *
     ...
     * {@code java.lang.String} (or {@code null} if none specified).
     *
     * @see SimpleDateFormat
     ...
     * <p>
     * <b>N.B.</b>If the converter has been configured to with one or more patterns
     * (using {@code setPatterns()}), then the first pattern will be used to format
     * the date into a String. Otherwise the default {@code DateFormat} for the
     * default locale (and <i>style</i> if configured) will be used.
     *
     ...
     * <p>
     * For {@code String} conversion, if the converter has been configured with one
     * or more patterns (using {@code setPatterns()}), then the conversion is
     * attempted with each of the specified patterns. Otherwise the default
     * {@code DateFormat} for the default locale (and <i>style</i> if configured)
     * will be used.
     *
     ...
     * </ul>
     * <p>
     * <strong>N.B.</strong> No default String conversion mechanism is provided for
     * {@code java.util.Date} and {@code java.util.Calendar} type.
     *
     * @param <T> The target type
     * @param type The date type to convert to
    ...
    /**
     * Parse a String into a {@code Calendar} object using the specified
     * {@code DateFormat}.
     *
     * @param sourceType The type of the value being converted
     * @param targetType The type to convert the value to
