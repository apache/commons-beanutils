 ...
 * </pre>
 * <p>
 * Converters generally treat null input as if it were invalid input, ie they
 * return their default value if one was specified when the converter was
 * constructed, and throw an exception otherwise. If you prefer nulls to be
 * preserved for converters that are converting to objects (not primitives) then
 * register a converter as above, passing a default value of null to the
 * converter constructor (and of course registering that converter only for the
 * .class target).
 * </p>
 * <p>
 * When a converter is listed above as having no default value, then that
 * converter will throw an exception when passed null or an invalid value as its
 * input. In particular, by default the BigInteger and BigDecimal converters
 * have no default (and are therefore somewhat inconsistent with the other
 * numerical converters which all have zero as their default).
 * </p>
 * <p>
 * Converters that generate <i>arrays</i> of each of the primitive types are
 * also automatically configured (including String[]). When passed null or
 * invalid input, these return an empty array (not null). See class
...
public class ConvertUtilsBean {
     ...
     * registered {@link Converter} for the {@code java.lang.String} class will be
     * used, which allows applications to customize Object-&gt;String conversions
     * (the default implementation simply uses toString()).
     *
     ...
     * class (if possible). If the specified Java class is itself an array class,
     * this class will be the type of the returned value. Otherwise, an array will
     * be constructed whose component type is the specified class.
     *
    ...
    /**
     * Convert the value to an object of the specified class (if possible). If no
     * converter for the desired target type is registered, the passed in object is
     * returned unchanged.
     *
     * @param value Value to be converted (may be null)
     ...
     *            exception when a conversion error occurs, otherwise {@code false}
     *            if a default value should be used.
     * @param defaultNull {@code true}if the <i>standard</i> converters (see
     *            {@link ConvertUtilsBean#registerStandard(boolean, boolean)})
     *            should use a default value of
     *            {@code null</code>, otherwise <code>false}.
     ...
     * 
     * @param throwException {@code true} if the converters should throw an
     *            exception when a conversion error occurs, otherwise <code>
     ...
     * &#64;param defaultArraySize The size of the default array value for array converters
     * (N.B. This values is ignored if {@code throwException</code> is <code>true}).
     * Specifying a value less than zero causes a <code>null<code> value to be used
     * for the default.
     */
     ...
     * that returns a default array of the specified size in the event of conversion
     * errors.
     *
     * @param componentType The component type of the array
     * @param componentConverter The converter to delegate to for the array elements
     * @param throwException Whether a conversion exception should be thrown or a
     *            default value used in the event of a conversion error
